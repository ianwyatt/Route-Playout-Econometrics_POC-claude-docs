...
The mobile index is controlled from the **sidebar** (collapsed by default). Two independent checkboxes — "Show Mean" and "Show Median" — toggle the index overlays across all tabs simultaneously.

| Tab | What's shown |
|-----|-------------|
| **Overview** | Dual-line campaign shape chart (blue raw, orange/purple dashed indexed) + indexed impacts row |
| **Executive Summary** | "Indexed Impacts" and "Indexed/Day" rows in delivery table + daily chart overlay |
| **Weekly Reach** | "Indexed (000s)" column in weekly table + coloured bar in grouped chart |
| **Daily & Hourly** | Indexed daily/weekly avg metrics + indexed bars in distribution + day-of-week charts |
| **Frame Audiences** | "Mobile Mean/Median Impacts" and "Mean/Median Index" columns in all 4 views |
| **Excel Export** | Includes indexed columns when either toggle is active |

### Visual conventions

- **Raw impacts**: solid blue line/bar (`#2E86AB`)
- **Mean indexed**: dashed orange line/bar (`#F18F01`)
- **Median indexed**: dotted purple line/bar (`#8E44AD`)
